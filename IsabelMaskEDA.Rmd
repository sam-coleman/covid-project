...
df_percent %>%
  filter(State == "CA") %>%
  arrange(date) %>%
  ggplot(mapping = aes(x = percent_mask, y = total_casesper100k)) +
  geom_segment(aes(xend = c(tail(percent_mask, n=-1), NA), yend = c(tail(total_casesper100k, n=-1), NA), color = State)) +
  scale_y_log10() +
  geom_point()
