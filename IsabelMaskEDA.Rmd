...
df_percent <- df_count %>%
  ...
  mutate(percent_mask = Yes / num_counties * 100)
df_percent
...
df_percent %>%
  filter(State == "MA") %>%
  arrange(date) %>%
  ggplot(mapping = aes(x = percent_mask, y = total_deathsper100k)) +
  geom_segment(aes(xend = c(tail(percent_mask, n=-1), NA), yend = c(tail(total_deathsper100k, n=-1), NA), color = State)) +
  scale_y_log10() +
  geom_point()
